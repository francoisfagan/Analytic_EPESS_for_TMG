function [ samples, num_fn_evals ] = epess_sampler( number_samples , dimension, number_chains, logLikelihood, EP_mean, EP_chol, initial_point )
% Generate samples using EPESS
    ...
    for chain_index = 1:number_chains
        ...
        for sample_index = 2:number_samples
            ...
            % Generate samples
            [samples(sample_index,:,chain_index), cur_log_like, cur_num_fn_evals, ~] = elliptical_slice( samples(sample_index-1,:,chain_index), EP_chol, pseudoLogLikelihoodShifted, cur_log_like);

            % Update number of function evaluations
            num_fn_evals = num_fn_evals + cur_num_fn_evals;
        end
